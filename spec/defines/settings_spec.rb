  ...
  on_supported_os.each do |os, os_facts|
    context "on #{os}" do
      ...
      context 'with minimal parameters' do
        ...
        it {
          is_expected.to create_ini_setting('/etc/dconf/db/gdm.d/enable_lock_delay [org/gnome/desktop/screensaver] lock-delay').with({
      ...
      context 'a setting with many items' do
        ...
        it {
          is_expected.to create_file('/etc/dconf/db/gdm.d/locks/set_wallpaper').with_content(<<-EOF.gsub(%r{^\s+}, '').strip,
      ...
      context 'a setting with many items, and one unlocked' do
        ...
        it {
          is_expected.to create_file('/etc/dconf/db/gdm.d/locks/arbitrary_name').with_content(<<-EOF.gsub(%r{^\s+}, '').strip,
        ...
        it {
          is_expected.to create_ini_setting('/etc/dconf/db/gdm.d/arbitrary_name [org/gnome/desktop/screensaver] lock-delay').with({
                                                                                                                                    section: 'org/gnome/desktop/screensaver',
          setting: 'lock-delay',
          value: true
                                                                                                                                  })
