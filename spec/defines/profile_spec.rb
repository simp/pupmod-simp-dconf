  ...
  on_supported_os.each do |os, os_facts|
    context "on #{os}" do
      ...
      context 'with a set of entries' do
        ...
        it {
          is_expected.to create_concat__fragment("dconf::profile::#{title}::user").with({
        ...
        it {
          is_expected.to create_concat__fragment("dconf::profile::#{title}::test").with({
            :target  => "#{params[:target]}/#{title}",
        ...
        it {
          is_expected.to create_concat__fragment("dconf::profile::#{title}::test2").with({
            :target  => "#{params[:target]}/#{title}",
            :order   => 15,
        ...
        it {
          is_expected.to create_concat__fragment("dconf::profile::#{title}::/some/file/path").with({
            :target  => "#{params[:target]}/#{title}",
            :order   => 15,
